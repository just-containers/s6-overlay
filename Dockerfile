...
# fix-attrs
ADD https://github.com/glerchundi/fix-attrs/releases/download/v0.4.0/fix-attrs-0.4.0-linux-amd64 /rootfs/usr/bin/fix-attrs
...
RUN tar xvfz /tmp/execline.tar.gz -C /rootfs
...
RUN tar xvfz /tmp/s6.tar.gz -C /rootfs
...
RUN tar xvfz /tmp/s6-portable-utils.tar.gz -C /rootfs

##
## INIT
##

RUN chmod +x /rootfs/etc/s6/.s6-init/init-stage*                               && \
    ln -s /rootfs/etc/s6/.s6-init/init-stage1 /rootfs/init                     && \
    ln -s /rootfs/etc/s6/.s6-init/init-stage3 /rootfs/etc/s6/.s6-svscan/finish
